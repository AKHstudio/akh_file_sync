    ...
    return (
        <>
            <nav className={`${styles.sidebar} ${isOpen ? styles.open : ''}`}>
                {navItems.map((item) => (
                    <Link key={item.href} href={item.href} className={`${styles.navLink} ${pathname === item.href ? styles.active : ''} ${item.sub ? styles.subLink : ''}`} onClick={() => setIsOpen(false)}>
                        {item.label}
                    </Link>
                ))}
            </nav>

            <div className={`${styles.sidebarOverlay} ${isOpen ? styles.active : ''}`} onClick={() => setIsOpen(false)} />

            <button className={styles.menuToggle} onClick={() => setIsOpen(!isOpen)} aria-label='メニュー'>
                {isOpen ? '✕' : '☰'}
            </button>
        </>
    );
